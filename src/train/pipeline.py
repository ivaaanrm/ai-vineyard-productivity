...
class TrainingPipeline:
    ...
    def _train_model(
        ...
        for target in self.config.targets:
            ...
            target_results[target] = {
                ...
                "test_within_10_pct": pct_within_test if test_metrics is not None else None,
        ...
        if self.config.mlflow.enabled:
            for target, info in target_results.items():
                self._log_mlflow_run(
                    model_name, target, info, feature_cols,
                    split_info, n_combos, experiment_dir, models_dir,
                )
    ...
    def _log_mlflow_run(
        ...
        with mlflow.start_run(run_name=f"{model_name}-{target}"):
            ...
            mlflow.log_metrics(
                {f"val_{k}": v for k, v in target_info["val_metrics"].items()}
            )
