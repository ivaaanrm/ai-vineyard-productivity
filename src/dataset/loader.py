...
class BandCube:
    ...
    def compute_indices(self, calculators: list) -> None:
        ...
        for calc in calculators:
            ...
            # Rebuild the dataset so the variable coordinate index is updated
            self._ds = xr.Dataset({"data": new_data}, attrs=self._ds.attrs)
            self._variables.append(calc.name)

    def __repr__(self) -> str:
        return (
            f"BandCube(sensor={self.sensor!r}, parcel={self.parcel_key!r}, "
            f"variables={self.variables}, n_times={len(self.times)})"
        )


@runtime_checkable
class BandLoaderProtocol(Protocol):
    """Protocol for loading band cubes — implement this to plug in a new data source."""

    def load(self, parcel_key: str, sensor: str) -> BandCube: ...
...
class ZarrBandLoader:
    ...
    def load(self, parcel_key: str, sensor: str) -> BandCube:
        ...
        return BandCube(ds, sensor=sensor, parcel_key=parcel_key)
